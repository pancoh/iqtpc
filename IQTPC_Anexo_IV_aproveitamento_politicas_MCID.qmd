...
As comparações do [documento principal](IQTPC_parametros_qualidade_transporte_publico.qmd) foram feitas com saúde e educação, setores maduros mas de outros ministérios, com outra lógica federativa e outros instrumentos de transferência. O saneamento oferece algo diferente e mais útil: **um precedente construído sob a mesma competência constitucional, com titularidade municipal, prestação delegada, regulação infranacional e dependência de transferência voluntária da União**, e operado pela secretaria vizinha, dentro do mesmo ministério.
...
Este é o elemento que resolve, a custo baixo, o problema de auditoria levantado no [documento principal](IQTPC_parametros_qualidade_transporte_publico.qmd). A lógica é descentralizar a verificação para a rede de reguladores infranacionais, em vez de custear auditoria federal direta em centenas de sistemas.
...
Isso é, funcionalmente, o **Cadastro Nacional de Sistemas de Transporte Público Coletivo** proposto como primeiro entregável no [Anexo II, seção 2.2](IQTPC_Anexo_II_pesquisa_nacional.qmd#primeiro-entregável-o-cadastro-do-universo). O modelo de dados, a lógica de responsabilidade pelo preenchimento e a arquitetura de validação podem ser adaptados em vez de concebidos do zero.
...
Note-se que a categoria de "soluções alternativas" do saneamento resolve o mesmo problema classificatório que o [Anexo II](IQTPC_Anexo_II_pesquisa_nacional.qmd) enfrentou ao propor as situações S1 a S4: como registrar atendimento precário ou não convencional sem tratá-lo como ausência de serviço nem como serviço regular.
...
1. **O valor está na série, não na edição.** Trinta anos de dado comparável tornaram o SNIS referência para gestores, reguladores, pesquisadores, instituições financeiras e organismos internacionais. Nenhuma edição isolada teria esse efeito. Isso reforça o princípio de sequenciamento do [documento principal](IQTPC_parametros_qualidade_transporte_publico.qmd#sequenciamento-de-implantação) (seção 12) e a exigência de imutabilidade do núcleo da pesquisa nacional.
...
| **Déficit de Acesso ao TPC**, número absoluto de pessoas sem serviço adequado | Déficit habitacional, Fundação João Pinheiro | Indicador satélite, [documento principal, 8.1](IQTPC_parametros_qualidade_transporte_publico.qmd#déficit-de-acesso-ao-transporte-público-coletivo) |
| **Norma de desempenho em vez de norma prescritiva** | NBR 15575 e PBQP-H | Forma das normas de referência, ver 3.5.2 |

A NBR 15575 também oferece limiares mensuráveis de conforto térmico e acústico. O componente correspondente não integra o índice objetivo, por ausência de fonte viável ([Anexo I](IQTPC_Anexo_I_matriz_origem_dados.qmd)), e o atributo é medido pela PeMOB Usuário.
